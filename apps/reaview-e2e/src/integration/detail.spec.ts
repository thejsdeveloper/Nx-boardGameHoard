describe('game review details', () => {
  ...
  it('should have a form to add a review', () => {
    ...
    cy.contains('label', 'Rating').find('input').type('3');
    cy.contains('label', 'Review').find('textarea').type(reviewText);
    cy.contains('button', 'Submit Review').click();

    cy.contains('label', 'Rating').find('input').should('have.value', '0');
    cy.contains('label', 'Review').find('textarea').should('have.value', '');
